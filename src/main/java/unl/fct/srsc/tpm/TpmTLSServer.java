...
public class TpmTLSServer {
    ...
    private static String buildResponse(String request) throws DecoderException, InvalidKeyException {
        String[] rq = request.split("\\|");
        try {

            if (rq[0].equals(REQUEST_CODE)) {
                ...
                BigInteger y = new BigInteger(rq[1]);
                ...
                return String.format("%s|%s|%s", RESPONSE_CODE, pubKey, tpmStatus);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
